...
func vlan_search(vlan string, allips []string, dbtype string, constring string) {
	...
	// }
	 ...
	 fmt.Println("EXCEPTION: VLAN 1008 - DO NOT ASSIGNED 10.10.8.252 or 10.10.15.240-254")
	 db.Close()
...
func dynamic_ip_search(ip string, dbtype string, constring string) {
        db, err := sql.Open(dbtype, constring)
        defer db.Close()
        results, err := db.Query("SELECT INET_NTOA(address), HEX(hwaddr), subnet_id, hostname FROM lease4 where INET_NTOA(address) =?", ip)
        for results.Next() {
                        rows++
                        var tag kea_entry
                        err = results.Scan(&tag.ipntoa, &tag.dehexmac, &tag.hname, &tag.dhcp_id)
...
func insert_bulk_record(ip string, vlan string, hostname string, mac string, dbtype string, constring string) {
	var dhcpidtype int = 0
        //insReader := bufio.NewReader(os.Stdin)
	fmt.Println("Connecting to databse", dbtype, "with", constring)
	db, err := sql.Open(dbtype, constring)
        defer db.Close()
        fmt.Println("Inserting record with: IP: ", ip, "\nVLAN: ", vlan, "\nMAC address: ", mac, "\nHostname: ", hostname, "\nDHCP ID Type: ", dhcpidtype)
        stmt, err := db.Prepare("INSERT INTO hosts (dhcp_identifier,dhcp_identifier_type,dhcp4_subnet_id,ipv4_address,hostname) VALUES (UNHEX(?),?,?,INET_ATON(?),?);")
...
func main() {
	...
	case "9":
		...
		dynamic_ip_search(ip, dbtype, constring)
        	os.Exit(1)
